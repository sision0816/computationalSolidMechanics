...
int main(int arc, char *argv[]) {
  ...
  const double beamDensity                = 1.0e-4;  // rho = 1.0e5 kgm^-3 // here should be 1.0e9 kgm^-3
  ...
  {
     // ... (Step 2-4)
    array<Node,SpatialDimension> nodesOfElement;
    for (unsigned int indexNode = 0; indexNode < SpatialDimension; indexNode++)
    {
      nodesOfElement[indexNode] = mesh._nodes[mesh._connectivity[elementIndex][indexNode]];
    }

    PhysicalElement physicalElement(nodesOfElement,
                                    elementProperties,
                                    & materialModel);

    physicalElementVector.push_back(physicalElement);


    ExternalElement externalElement(physicalElement,bodyForceVector);

    externalElementVector.push_back(externalElement);
  ...
  for (unsigned int nodeIndex = 0; nodeIndex < numberOfNodes; nodeIndex++){ // (Step 1 - given)
         Point nodePosition = mesh._nodes[nodeIndex]._position;

        if ( abs(nodePosition(1) - meshMinPosition(1)) < nodePositionSearchTolerance)
         {
  ...
  ignoreUnusedVariables(nodePositionSearchTolerance);
  ...
  const vector<Stress> elementStresses =  physicalAssembler.computeElementStresses(displacements);
