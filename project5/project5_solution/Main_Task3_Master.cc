...
int main(int arc, char *argv[]) {
  ...
  for (unsigned int indexElement = 0; indexElement < (mesh._connectivity).size(); indexElement++)
  {
    ...
    Element simplexElement(	nodesSimplex		,
		...
		// Push the newly created Element into elements.
  ...
  const unsigned int  numberOfLoadsteps = 1000;
  const double        maxTension        = 0.2 ;
  vector<Vector>      initialGuess(numberOfNodes, Vector::Zero());
  
  double appliedDeformation
    = 0.;
  const double increment
    = maxTension/(double)numberOfLoadsteps;
  ...
  for (unsigned int loadstepIndex = 0; loadstepIndex < numberOfLoadsteps; loadstepIndex++) {
    ...
    const double spatialBoundaryConditionTolerance = 1.e-4;  
    for (unsigned int nodeIndex = 0; nodeIndex < numberOfNodes; nodeIndex++)
    {
      const Point nodePosition = mesh._nodes[nodeIndex]._position;
      
      if (nodePosition(0) < meshMinPosition(0) + spatialBoundaryConditionTolerance) 
      {
        essentialBCs.push_back(EssentialBoundaryCondition(nodeIndex, 0, 0));
        essentialBCs.push_back(EssentialBoundaryCondition(nodeIndex, 1, 0));
      }
      if (nodePosition(0) > meshMaxPosition(0) - spatialBoundaryConditionTolerance) 
      {
        essentialBCs.push_back(EssentialBoundaryCondition(nodeIndex, 0, appliedDeformation));
        essentialBCs.push_back(EssentialBoundaryCondition(nodeIndex, 1, 0));
      }
    }
    ...
    // Use the Newton-Raphson solver that uses the assembler and the
