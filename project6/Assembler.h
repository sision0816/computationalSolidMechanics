...
public:
  ...
  typedef typename Element::MassMatrix                   ElementMassMatrix;
  ...
  assembleConsistentMassMatrix() const {
    ...
    // TODO: Fill in the consistent mass matrix - as mentioned in the project assignment
    //       there may be a lot of similarities to the assembly of the stiffness matrix
    for (size_t elementIndex = 0; elementIndex < _elements.size(); ++elementIndex) {
      ...
      array<size_t, NumberOfNodesPerElement> elementNodeIds =
        element.getNodeIds();
      ElementMassMatrix elementMassMatrix =
        element.computeConsistentMassMatrix();
        ...
        for (size_t nodeIndex2 = 0; nodeIndex2 < elementNodeIds.size();
          ...
          for (size_t i = 0; i < DegreesOfFreedom; ++i) {
            for (size_t j = 0; j < DegreesOfFreedom; ++j) {
              consistentMassMatrix(nodeId1 * DegreesOfFreedom + i,
                              nodeId2 * DegreesOfFreedom + j) +=
                elementMassMatrix(nodeIndex1 * DegreesOfFreedom + i,
  ...
  assembleStiffnessMatrix(const vector<ElementVector> & displacements) const {
    ...
    // normal elements
    for (size_t elementIndex = 0; elementIndex < _elements.size();
         ++elementIndex) {
      const Element & element = _elements[elementIndex];
      array<size_t, NumberOfNodesPerElement> elementNodeIds =
        element.getNodeIds();
